...
{	//读取MP3文件中的信息
	...
	id3file = id3_file_open(file , ID3_FILE_MODE_READONLY);	//打开文件并创建id3_file结构体
	if(NULL == id3file) {		//判断文件打开是否失败
		printf("打开文件失败\n");
		return -1;
	}
	tag = id3_file_tag(id3file);	//创建id3_tag结构体
	frame_title = id3_tag_findframe(tag, ID3_FRAME_TITLE, 0);	//创建id3_frame结构体
	if(frame_title -> fields) {	//防止指向NULL的地址操作
		strcpy(media -> title, (char *) frame_title -> fields);	//将项目信息复制到媒体库结构体
	}
	...
	if(frame_time -> fields) {
		media -> record_time = id3_field_getint(frame_time -> fields);	//将项目信息作为数字类型提取
	}
